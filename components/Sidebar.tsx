"use client";

import Link from "next/link";
import Image from "next/image";
import { usePathname } from "next/navigation";
import { useSidebar } from "@/store/sidebar-context";
import { cn } from "@/lib/utils";

export default function Sidebar() {
  const pathname = usePathname();
  let collapsed = false;

  try {
    const context = useSidebar();
    collapsed = context.collapsed;
  } catch (error) {
    console.error("Sidebar context not available:", error);
    // Use default values for collapsed
  }

  return (
    <aside
      className={cn(
        "h-screen bg-white border-r border-gray-100 flex flex-col transition-all duration-300",
        collapsed ? "w-16" : "w-72"
      )}
    >
      {/* Header */}
      <div className={cn("p-4 pb-6", collapsed && "flex justify-center")}>
        <div
          className={cn(
            "flex items-center gap-3 mb-2",
            collapsed && "justify-center"
          )}
        >
          <div className="bg-black rounded-lg p-2 w-10 h-10 flex items-center justify-center flex-shrink-0">
            <svg
              width="20"
              height="20"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M3 10H21M7 15H8M12 15H13M6 19H18C19.6569 19 21 17.6569 21 16V8C21 6.34315 19.6569 5 18 5H6C4.34315 5 3 6.34315 3 8V16C3 17.6569 4.34315 19 6 19Z"
                stroke="white"
                strokeWidth="2"
                strokeLinecap="round"
                strokeLinejoin="round"
              />
            </svg>
          </div>
          {!collapsed && (
            <div>
              <h1 className="font-medium text-lg text-gray-900">Bucx</h1>
              <p className="text-xs text-gray-500">Admin Dashboard</p>
            </div>
          )}
        </div>
      </div>

      {/* Navigation Sections */}
      <nav className="flex-1 overflow-y-auto">
        {!collapsed && (
          <p className="px-4 text-sm text-gray-500 font-normal">Home</p>
        )}
        <ul
          className={cn("mt-2 mb-6", collapsed && "flex flex-col items-center")}
        >
          <li className="w-full">
            <Link
              href="/admin/dashboard"
              className={cn(
                "flex items-center gap-3 py-2 text-gray-700 hover:bg-gray-100",
                collapsed ? "px-2 justify-center" : "px-4",
                pathname === "/admin/dashboard" && "bg-gray-100"
              )}
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
                strokeWidth={1.5}
                stroke="currentColor"
                className="size-6"
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  d="m6.75 7.5 3 2.25-3 2.25m4.5 0h3m-9 8.25h13.5A2.25 2.25 0 0 0 21 18V6a2.25 2.25 0 0 0-2.25-2.25H5.25A2.25 2.25 0 0 0 3 6v12a2.25 2.25 0 0 0 2.25 2.25Z"
                />
              </svg>
              {!collapsed && <span>Dashboard</span>}
            </Link>
          </li>
        </ul>

        {!collapsed && (
          <p className="px-4 text-sm text-gray-500 font-normal">Transactions</p>
        )}
        <ul
          className={cn("mt-2 mb-6", collapsed && "flex flex-col items-center")}
        >
          <li className="w-full">
            <Link
              href="/admin/deposits"
              className={cn(
                "flex items-center gap-3 py-2 text-gray-700 hover:bg-gray-100",
                collapsed ? "px-2 justify-center" : "px-4",
                pathname === "/admin/deposits" && "bg-gray-100"
              )}
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
                strokeWidth={1.5}
                stroke="currentColor"
                className="size-6"
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  d="M3 13.125C3 12.504 3.504 12 4.125 12h2.25c.621 0 1.125.504 1.125 1.125v6.75C7.5 20.496 6.996 21 6.375 21h-2.25A1.125 1.125 0 0 1 3 19.875v-6.75ZM9.75 8.625c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125v11.25c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 0 1-1.125-1.125V8.625ZM16.5 4.125c0-.621.504-1.125 1.125-1.125h2.25C20.496 3 21 3.504 21 4.125v15.75c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 0 1-1.125-1.125V4.125Z"
                />
              </svg>
              {!collapsed && <span>Deposits</span>}
            </Link>
          </li>
          <li className="w-full">
            <Link
              href="/admin/withdrawals"
              className={cn(
                "flex items-center gap-3 py-2 text-gray-700 hover:bg-gray-100",
                collapsed ? "px-2 justify-center" : "px-4",
                pathname === "/admin/withdrawals" && "bg-gray-100"
              )}
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
                strokeWidth={1.5}
                stroke="currentColor"
                className="size-6"
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  d="M2.25 8.25h19.5M2.25 9h19.5m-16.5 5.25h6m-6 2.25h3m-3.75 3h15a2.25 2.25 0 0 0 2.25-2.25V6.75A2.25 2.25 0 0 0 19.5 4.5h-15a2.25 2.25 0 0 0-2.25 2.25v10.5A2.25 2.25 0 0 0 4.5 19.5Z"
                />
              </svg>
              {!collapsed && <span>Withdrawals</span>}
            </Link>
          </li>
          <li className="w-full">
            <Link
              href="/admin/transaction-logs"
              className={cn(
                "flex items-center gap-3 py-2 text-gray-700 hover:bg-gray-100",
                collapsed ? "px-2 justify-center" : "px-4",
                pathname === "/admin/transaction-logs" && "bg-gray-100"
              )}
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
                strokeWidth={1.5}
                stroke="currentColor"
                className="size-6"
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  d="M9 6.75V15m6-6v8.25m.503 3.498 4.875-2.437c.381-.19.622-.58.622-1.006V4.82c0-.836-.88-1.38-1.628-1.006l-3.869 1.934c-.317.159-.69.159-1.006 0L9.503 3.252a1.125 1.125 0 0 0-1.006 0L3.622 5.689C3.24 5.88 3 6.27 3 6.695V19.18c0 .836.88 1.38 1.628 1.006l3.869-1.934c.317-.159.69-.159 1.006 0l4.994 2.497c.317.158.69.158 1.006 0Z"
                />
              </svg>
              {!collapsed && <span>Transaction logs</span>}
            </Link>
          </li>
        </ul>

        {!collapsed && (
          <p className="px-4 text-sm text-gray-500 font-normal">
            User Management
          </p>
        )}
        <ul
          className={cn("mt-2 mb-6", collapsed && "flex flex-col items-center")}
        >
          <li className="w-full">
            <Link
              href="/admin/users"
              className={cn(
                "flex items-center gap-3 py-2 text-gray-700 hover:bg-gray-100",
                collapsed ? "px-2 justify-center" : "px-4",
                pathname === "/admin/users" && "bg-gray-100"
              )}
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
                strokeWidth={1.5}
                stroke="currentColor"
                className="size-6"
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  d="M18 18.72a9.094 9.094 0 0 0 3.741-.479 3 3 0 0 0-4.682-2.72m.94 3.198.001.031c0 .225-.012.447-.037.666A11.944 11.944 0 0 1 12 21c-2.17 0-4.207-.576-5.963-1.584A6.062 6.062 0 0 1 6 18.719m12 0a5.971 5.971 0 0 0-.941-3.197m0 0A5.995 5.995 0 0 0 12 12.75a5.995 5.995 0 0 0-5.058 2.772m0 0a3 3 0 0 0-4.681 2.72 8.986 8.986 0 0 0 3.74.477m.94-3.197a5.971 5.971 0 0 0-.94 3.197M15 6.75a3 3 0 1 1-6 0 3 3 0 0 1 6 0Zm6 3a2.25 2.25 0 1 1-4.5 0 2.25 2.25 0 0 1 4.5 0Zm-13.5 0a2.25 2.25 0 1 1-4.5 0 2.25 2.25 0 0 1 4.5 0Z"
                />
              </svg>
              {!collapsed && <span>Users</span>}
            </Link>
          </li>
          <li className="w-full">
            <Link
              href="/admin/withdrawals-users"
              className={cn(
                "flex items-center gap-3 py-2 text-gray-700 hover:bg-gray-100",
                collapsed ? "px-2 justify-center" : "px-4",
                pathname === "/admin/withdrawals-users" && "bg-gray-100"
              )}
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
                strokeWidth={1.5}
                stroke="currentColor"
                className="size-6"
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  d="M2.25 8.25h19.5M2.25 9h19.5m-16.5 5.25h6m-6 2.25h3m-3.75 3h15a2.25 2.25 0 0 0 2.25-2.25V6.75A2.25 2.25 0 0 0 19.5 4.5h-15a2.25 2.25 0 0 0-2.25 2.25v10.5A2.25 2.25 0 0 0 4.5 19.5Z"
                />
              </svg>
              {!collapsed && <span>Withdrawals</span>}
            </Link>
          </li>
          <li className="w-full">
            <Link
              href="/admin/transaction-logs-users"
              className={cn(
                "flex items-center gap-3 py-2 text-gray-700 hover:bg-gray-100",
                collapsed ? "px-2 justify-center" : "px-4",
                pathname === "/admin/transaction-logs-users" && "bg-gray-100"
              )}
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
                strokeWidth={1.5}
                stroke="currentColor"
                className="size-6"
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  d="M9 6.75V15m6-6v8.25m.503 3.498 4.875-2.437c.381-.19.622-.58.622-1.006V4.82c0-.836-.88-1.38-1.628-1.006l-3.869 1.934c-.317.159-.69.159-1.006 0L9.503 3.252a1.125 1.125 0 0 0-1.006 0L3.622 5.689C3.24 5.88 3 6.27 3 6.695V19.18c0 .836.88 1.38 1.628 1.006l3.869-1.934c.317-.159.69-.159 1.006 0l4.994 2.497c.317.158.69.158 1.006 0Z"
                />
              </svg>
              {!collapsed && <span>Transaction logs</span>}
            </Link>
          </li>
          <li className="w-full">
            <Link
              href="/admin/waitlist-users"
              className={cn(
                "flex items-center gap-3 py-2 hover:bg-gray-100",
                collapsed ? "px-2 justify-center" : "px-4",
                pathname === "/admin/waitlist-users"
                  ? "bg-gray-900 text-white"
                  : "text-gray-700"
              )}
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
                strokeWidth={1.5}
                stroke="currentColor"
                className="size-6"
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  d="M15 19.128a9.38 9.38 0 0 0 2.625.372 9.337 9.337 0 0 0 4.121-.952 4.125 4.125 0 0 0-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 0 1 8.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0 1 11.964-3.07M12 6.375a3.375 3.375 0 1 1-6.75 0 3.375 3.375 0 0 1 6.75 0Zm8.25 2.25a2.625 2.625 0 1 1-5.25 0 2.625 2.625 0 0 1 5.25 0Z"
                />
              </svg>
              {!collapsed && <span>Waitlist users</span>}
            </Link>
          </li>
        </ul>

        {!collapsed && (
          <p className="px-4 text-sm text-gray-500 font-normal">
            Account Management
          </p>
        )}
        <ul
          className={cn("mt-2 mb-6", collapsed && "flex flex-col items-center")}
        >
          <li className="w-full">
            <Link
              href="/admin/usd-account"
              className={cn(
                "flex items-center gap-3 py-2 text-gray-700 hover:bg-gray-100",
                collapsed ? "px-2 justify-center" : "px-4",
                pathname === "/admin/usd-account" && "bg-gray-100"
              )}
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
                strokeWidth={1.5}
                stroke="currentColor"
                className="size-6"
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  d="M12 6v12m-3-2.818.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z"
                />
              </svg>
              {!collapsed && <span>USD account</span>}
            </Link>
          </li>
          <li className="w-full">
            <Link
              href="/admin/solana-wallet"
              className={cn(
                "flex items-center gap-3 py-2 text-gray-700 hover:bg-gray-100",
                collapsed ? "px-2 justify-center" : "px-4",
                pathname === "/admin/solana-wallet" && "bg-gray-100"
              )}
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
                strokeWidth={1.5}
                stroke="currentColor"
                className="size-6"
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  d="M21 12a2.25 2.25 0 0 0-2.25-2.25H15a3 3 0 1 1-6 0H5.25A2.25 2.25 0 0 0 3 12m18 0v6a2.25 2.25 0 0 1-2.25 2.25H5.25A2.25 2.25 0 0 1 3 18v-6m18 0V9M3 12V9m18 0a2.25 2.25 0 0 0-2.25-2.25H5.25A2.25 2.25 0 0 0 3 9m18 0V6a2.25 2.25 0 0 0-2.25-2.25H5.25A2.25 2.25 0 0 0 3 6v3"
                />
              </svg>
              {!collapsed && <span>Solana wallet</span>}
            </Link>
          </li>
          <li className="w-full">
            <Link
              href="/admin/transaction-logs-account"
              className={cn(
                "flex items-center gap-3 py-2 text-gray-700 hover:bg-gray-100",
                collapsed ? "px-2 justify-center" : "px-4",
                pathname === "/admin/transaction-logs-account" && "bg-gray-100"
              )}
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
                strokeWidth={1.5}
                stroke="currentColor"
                className="size-6"
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  d="M9 6.75V15m6-6v8.25m.503 3.498 4.875-2.437c.381-.19.622-.58.622-1.006V4.82c0-.836-.88-1.38-1.628-1.006l-3.869 1.934c-.317.159-.69.159-1.006 0L9.503 3.252a1.125 1.125 0 0 0-1.006 0L3.622 5.689C3.24 5.88 3 6.27 3 6.695V19.18c0 .836.88 1.38 1.628 1.006l3.869-1.934c.317-.159.69-.159 1.006 0l4.994 2.497c.317.158.69.158 1.006 0Z"
                />
              </svg>
              {!collapsed && <span>Transaction logs</span>}
            </Link>
          </li>
          <li className="w-full">
            <Link
              href="/admin/waitlist-users-account"
              className={cn(
                "flex items-center gap-3 py-2 text-gray-700 hover:bg-gray-100",
                collapsed ? "px-2 justify-center" : "px-4",
                pathname === "/admin/waitlist-users-account" && "bg-gray-100"
              )}
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
                strokeWidth={1.5}
                stroke="currentColor"
                className="size-6"
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  d="M9 6.75V15m6-6v8.25m.503 3.498 4.875-2.437c.381-.19.622-.58.622-1.006V4.82c0-.836-.88-1.38-1.628-1.006l-3.869 1.934c-.317.159-.69.159-1.006 0L9.503 3.252a1.125 1.125 0 0 0-1.006 0L3.622 5.689C3.24 5.88 3 6.27 3 6.695V19.18c0 .836.88 1.38 1.628 1.006l3.869-1.934c.317-.159.69-.159 1.006 0l4.994 2.497c.317.158.69.158 1.006 0Z"
                />
              </svg>
              {!collapsed && <span>Waitlist users</span>}
            </Link>
          </li>
        </ul>
      </nav>

      {/* User Profile */}
      <div className="border-t border-gray-100 p-4">
        {collapsed ? (
          <div className="flex justify-center">
            <div className="h-10 w-10 rounded overflow-hidden">
              <Image
                src="/flags/uk.png"
                alt="User Avatar"
                width={40}
                height={40}
                className="object-cover"
              />
            </div>
          </div>
        ) : (
          <div className="flex items-center gap-3">
            <div className="h-10 w-10 rounded overflow-hidden">
              <Image
                src="/flags/uk.png"
                alt="User Avatar"
                width={40}
                height={40}
                className="object-cover"
              />
            </div>
            <div className="flex-1">
              <h3 className="font-medium text-gray-900">Slazenger</h3>
              <p className="text-xs text-gray-500">slazenger@gmail.com</p>
            </div>
          </div>
        )}
      </div>
    </aside>
  );
}
